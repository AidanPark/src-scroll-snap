...
| 항목 | 값 | 근거 |
|------|---|------|
| 격자 해상도 | 96 × 54 셀 | 1920÷20=96, 1080÷20=54. 각 셀은 화면에서 20×20 픽셀로 확대 표시 |
| 색상 수 | 4색 (2비트/셀) | MJPEG 압축 아티팩트에도 안정적 구분 가능 |
| 프레임당 셀 수 | 5,184 | 96 × 54 |
| 프레임 총 용량 | 1,296 바이트 | 5,184 셀 ÷ 4 = 1,296B. 전체 버전: 헤더 8B + 데이터 1,288B |
| 프레임 전환 간격 | 500ms (2fps) | PowerShell/브라우저 렌더링 안정성 |
| 화면 표시 | 전체 화면, NearestNeighbor 확대 | `image-rendering: pixelated` |
| 최대 파일 크기 | 실용 상한 ~50MB | 소스 HTML에 base64(×1.33)를 삽입하므로 브라우저 메모리 소비 ≈ 원본×2.5배. 탭 메모리 한계는 브라우저·OS·버전에 따라 다름(통상 1~4GB). 다만 5MB 전송에 ~1시간 소요되므로 실사용 범위에서는 메모리보다 시간이 제약 |
...
HDMI → MJPEG 캡처 체인에서 안정적으로 구분 가능한 4색을 선택한다. **휘도(luma) 차이가 최대**인 조합:
...
> 4색 모두 휘도 차이가 크므로, JPEG 4:2:0 크로마 서브샘플링에서도 구분 가능하다.
...
헤더 구조 (8바이트 = 32셀):
  ...
  바이트 6: 파일 해시 상위 바이트 (SHA-256 처음 2바이트)
  바이트 7: 파일 해시 하위 바이트
...
- 타겟 측 디코딩: 프레임 순서 추적을 타이틀 바에서 읽거나, 내용 해시로 중복 제거
...
캡처 프레임 (1920×1080 MJPEG)
    ↓
[1단계] 격자 감지
    - 화면에서 96×54 격자의 위치/크기 탐지
    - 밝기 변화 패턴으로 블록 경계 추정
    - 또는: 고정 위치 가정 (전체 화면 모드이므로)
    ↓
[2단계] 색상 샘플링
    - 각 블록의 중앙 영역(예: 10×10 픽셀)의 평균 RGB 추출
...
> 소스 PC의 타이핑 제약을 고려하면, **프레임 반복**이 파운틴 코드보다 훨씬 실용적이다. 파운틴 코드는 소스 측에 수백 줄의 인코딩 코드가 필요하지만, 프레임 반복은 단순 루프만으로 동일한 효과를 얻는다.
...
```
96×54 격자, 4색, 2fps, 프레임당 1,288 바이트 (전체 버전):

1주기 소요 시간:
  T = ceil(파일크기 / 1,288) × 0.5초

반복 고려 (2주기로 완료 가정):
  실효 전송 속도 ≈ 1,288 × 2fps / 2 ≈ 1,288 B/s ≈ 약 1.3 KB/s
...
| 파일 크기 | 프레임 수 | 1주기 시간 | 예상 완료 (2주기) | 현실성 |
|----------|----------|----------|-----------------|--------|
| 10 KB | 8 | 4초 | ~8초 | 즉시 |
| 100 KB | 80 | 40초 | ~1분 20초 | 실용적 |
| 500 KB | 398 | 3분 19초 | ~6분 38초 | 실용적 |
| 1 MB | 815 | 6분 48초 | ~13분 36초 | 가능 |
| 5 MB | 4,071 | 33분 56초 | ~1시간 8분 | 고됨 |
...
격자의 각 셀(20×20 화면 픽셀)에서 중앙 10×10 영역만 샘플링한다. JPEG 아티팩트는 블록 경계에 집중되므로, 중앙부는 깨끗하다.
...
### 7.3 NearestNeighbor 스케일링 보장

소스 PC에서 96×54 Canvas를 CSS로 전체 화면 확대할 때, `image-rendering: pixelated`를 지정하여 블록 경계가 선명하게 유지되도록 한다:
...
| 특정 프레임이 반복적으로 실패 | 완료 불가 | 타겟 PC에서 문제 프레임 번호 표시. 소스 PC에서 스크립트 재실행하여 오프셋 변경 |
...
4. **소형 비트맵 + 확대**: 96×54 Canvas를 전체 화면으로 확대하여, 캡처 시 20×20 픽셀 블록 생성
