...
> **구현 상태 (v0.2.6)**: 본 문서의 텍스트 캡처 기능은 독립 탭이 아닌 **스크롤 캡처(SC) 모드에 통합**되어 구현되었다. 스크롤 캡처 + OCR 파이프라인으로 텍스트를 추출한다. 아래 세부 사양 중 구현 상태는 각 항목에 ✅/[부분 구현]/[미구현]으로 표시한다.
...
[4단계] OCR 수행
    ...
    > ✅ 구현됨 (Tesseract.js + Google Cloud Vision + LLM Vision)
    ↓
[5단계] 텍스트 후처리
...
> **[미구현]** v0.2.6에서는 인터랙티브 그리드 오버레이 대신 퍼센트 기반 크롭 영역(상/하/좌/우 퍼센트)으로 OCR 대상 영역을 지정한다.
...
> **[부분 구현]** v0.2.6에서는 chunkCanvas를 사용한 이미지 분할이 구현되었으나, 줄 경계 감지(밝기 변화 기반) 분할은 미구현이다.
...
> **[미구현]** v0.2.6에서는 OCR 결과를 그대로 사용하며, 중복 줄 제거/줄 번호 제거/공백 정규화 등 후처리 파이프라인은 미구현이다.
...
> **[미구현]** v0.2.6에서는 스크롤 속도 대응 전략(적응형 샘플링, 속도 감지, 경고, 겹침 검증, 복구)이 전체적으로 미구현이다. 기본 NCC 매칭으로만 동작한다.
...
> **[부분 구현]** v0.2.6에서는 점진적 스트립 추출(프레임 2장 유지) + Blob 오프로딩(BLOB_THRESHOLD: 100 스트립)이 구현되었다. 메모리 예산 경고/자동 중단 계층은 미구현이다.
...
| LLM Vision OCR | OCR 모듈 ✅ | ✅ 구현됨 (OCR 드롭다운 + 키 관리) |
| Google Cloud Vision OCR | 없음 | ✅ 구현됨 |
| Tesseract.js OCR | OCR 모듈 ✅ | 설정 조정만 |
| 결과 텍스트 편집/다운로드 | OCR 모듈 ✅ | 없음 |
| OCR 타겟 영역 선택 그리드 | 없음 | **신규** [미구현] |
| OCR 이미지 분할/병합 | 없음 | [부분 구현] chunkCanvas |
| 메모리 관리 / Blob 오프로딩 | 없음 | [부분 구현] 스트립 추출 + Blob 오프로딩 |
| 텍스트 후처리 (중복 제거 등) | 없음 | **신규** [미구현] |
| 스크롤 속도 적응 제어 | 없음 | **신규** [미구현] |
...
| 순서 | 단계 | 내용 | 의존성 | 난이도 |
|------|------|------|--------|--------|
| 1 | 스크롤 캡처 완성 | 스크롤 캡처 + 이미지 스티칭 | 없음 | ✅ 완료 |
| 2 | OCR 엔진 통합 | Tesseract + Google Cloud Vision OCR + 멀티모달 LLM 제공자 연동 | 단계 1 | ✅ 완료 |
| 3 | OCR 타겟 영역 선택 그리드 | 라이브 프리뷰 오버레이 + 드래그 ROI + 프리셋 재사용 | 단계 1 | [미구현] |
| 4 | 메모리 관리 계층 도입 | 점진적 스트립 추출 + Blob 오프로딩 + 메모리 예산 경고/자동 중단 | 단계 1 | [부분 구현] |
| 5 | 이미지 분할 OCR | 긴 이미지를 줄 경계에서 분할 → OCR → 병합 | 단계 2, 4 | [부분 구현] |
| 6 | 텍스트 후처리 | 중복 제거, 줄 번호 제거, 공백 정규화 | 단계 5 | [미구현] |
| 7 | 스크롤 속도 대응 | 속도 추정, 적응 샘플링, 경고/복구 UI | 단계 1 | [미구현] |
| 8 | 결과 UI | 텍스트 편집기 + 다운로드 + 클립보드 복사 | 단계 6 | ✅ 완료 |
| 9 | 정확도 테스트 | 다양한 폰트/테마/언어/에디터 조합 테스트 | 전체 | [미구현] |
