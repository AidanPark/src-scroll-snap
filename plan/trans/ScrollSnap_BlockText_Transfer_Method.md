...
| 채도 증가 감지 | frame_0과 frame_2 비교 → 채도가 증가한(saturation increase > 0.12 AND 절대 채도 > 0.15) 픽셀만 하이라이트로 판정 | 커서 깜박임·시계·애니메이션 등 저채도 변화 무시, 선택 하이라이트만 식별 |
...
- Step 2 채도 증가 감지: frame_0 vs frame_2 비교, 채도 증가 > 0.12 AND 절대 채도 > 0.15인 픽셀 20개 이상
...
> 코드에서는 `stabilityThreshold` 파라미터(기본 1.0초, 0.5~3.0초 조절 가능)로 캡처 간격을 설정한다.
...
| 기능 | 동작 |
|------|------|
| 중복 방지 | 직전 OCR 결과와 정확 문자열 비교(trim 후 완전 일치) — 동일하면 새 항목 생성 대신 기존 항목의 타임스탬프만 갱신 |
...
| OCR 후 쿨다운 | OCR 완료 후 5초간 감지 억제 + 프레임 버퍼 초기화. 선택 해제 시 재트리거 방지 |
...
[Step 2: 채도 증가 감지]
   ...
   - 채도 증가 > 0.12 AND 절대 채도 > 0.15인 픽셀 추출
...
| Step 2 | 채도 증가 픽셀 수 (frame_0 vs frame_2) | ≥ 20 (satIncrease > 0.12, satAbsolute > 0.15) | 하이라이트 없음 → 대기 |
...
flowchart TD
    ...
    C -->|불안정| B
    C -->|안정| D{Step 2: 채도 증가 감지<br>satIncrease > 0.12?}
    D -->|하이라이트 없음| B
...
| OpenAI | GPT-4o | 이미지+텍스트 동시 입력 OCR 품질 우수, 응답 속도 안정 |
| Anthropic | Claude 4 Sonnet | 장문 정리 및 문맥 보정 강점. `Claude Max`는 claude.ai 앱 구독이며 API 사용량/요금과는 별도 |
