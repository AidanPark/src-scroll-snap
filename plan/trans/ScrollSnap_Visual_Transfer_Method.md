...
sequenceDiagram
    ...
    U->>S: PowerShell 스크립트 타이핑 (notepad, ~900-1000자)
...
> **현재 구현 상태**: v0.2.6 기준 Standard 클래스만 구현되어 있다. Enhanced/Maximum 클래스는 향후 확장 예정이다.
...
1. 파일 읽기 → 바이트 배열
2. 파일명 헤더 생성: 원본 파일명을 UTF-8로 인코딩하고, [1바이트 길이] + [파일명 바이트] + [파일 데이터] 형태로 결합 (§3.6 참조)
...
소스 PC 스크립트는 원본 파일명을 데이터 앞에 삽입하여 타겟 PC에서 파일명을 복원할 수 있도록 한다.

**데이터 레이아웃:**

```
[1바이트: 파일명 길이 N (UTF-8 바이트 수)][N바이트: 파일명 (UTF-8)][나머지: 원본 파일 데이터]
```

| 오프셋 | 크기 | 설명 |
|--------|------|------|
| 0 | 1 바이트 | 파일명 길이 N (UTF-8 인코딩 후 바이트 수) |
| 1 | N 바이트 | 파일명 (UTF-8) |
| 1+N | 나머지 | 원본 파일 데이터 |
...
```
파일명: "report.pptx" (UTF-8: 12바이트)
→ 0x0C | "report.pptx" (UTF-8) | <파일 바이너리 데이터>
```

**폴백 규칙:**
- 파일명 길이가 0이거나 200을 초과하면 파일명이 없는 것으로 간주
- 파일명 없는 경우 타겟 PC에서 `transfer_YYYYMMDD_HHmmss.bin` 형식으로 자동 생성

> 이 프로토콜은 §4.2 전체 버전과 §4.3 WinForms 폴백 버전 모두에 적용된다.
...
Step 3: 스크립트 실행
  > powershell -ep bypass -f $env:TEMP\t.ps1 C:\path\to\file.pptx
...
프레임 헤더(매직넘버, 프레임 번호, 총 프레임 수)와 파일명 보존 기능을 포함하는 정식 버전.
...
- 프레임 헤더: 매직넘버(SC) + 프레임 번호 + 총 프레임 수
- 타깃 측에서 프레임 식별, 중복 제거, 완료 감지 가능
- 파일명 보존: 원본 파일명을 데이터 앞에 삽입 (§3.6 참조)

### 4.3 WinForms 폴백 버전 (약 750자)
...
**실행**: `powershell -sta -ep bypass -f $env:TEMP\t.ps1 C:\file.pptx`
...
[7단계] 파일 복원 (모든 프레임 수집 완료 시)
    ...
    - 파일명 추출: 데이터 앞에서 파일명 길이 + 파일명 바이트 분리 (§3.6 참조)
    - 원본 파일 다운로드
```
...
캡처 프레임에서 96×54 격자의 위치를 자동으로 감지하는 알고리즘:

```
1. 프레임을 그레이스케일로 변환
2. 상단→하단 스캔: 밝기 임계치를 초과하는 첫 번째 행 = 격자 상단 경계
3. 좌측→우측 스캔: 밝기 임계치를 초과하는 첫 번째 열 = 격자 좌측 경계
4. 하단→상단, 우측→좌측 동일 스캔으로 하단/우측 경계 결정
5. 격자 영역 크기에서 셀 크기 계산: cellW = width / 96, cellH = height / 54
6. 16:9 비율 검증 (허용 오차 5%)
7. 검증 실패 시 전체 프레임을 격자 영역으로 가정 (폴백)
```

> 전체 화면(F11) 모드에서는 격자가 프레임 전체를 채우므로 감지가 단순하다. 비전체화면이나 DPI 스케일링이 적용된 경우 자동 감지가 유용하다.
...
3. **최소 타이핑**: 소스 PC 스크립트를 약 900~1,000자로 최소화. 8~12분 타이핑으로 실행 가능
